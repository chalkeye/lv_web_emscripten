...
// ============================================================================
// 1. UI & LANGUAGE BRIDGE (Works for Web & Hardware)
// ============================================================================
#ifdef __cplusplus
extern "C" {
#endif
...
// ============================================================================
// 2. HARDWARE INCLUDES & DEFINES (ESP32-S3 ONLY)
// ============================================================================
#ifdef ARDUINO
    #include <Arduino.h>
...
// ============================================================================
// 3. WEB SIMULATOR INCLUDES & DEFINES (Emscripten ONLY)
// ============================================================================
#else
...
// ============================================================================
// 4. KAFFELOGIC UI LOGIC & STATE MACHINE (100% SHARED - YOUR ORIGINAL CODE)
// ============================================================================
...
void run_system_loop() {
        ...
        if (lv_scr_act() == ui_RoastMain) {
            if (current_time - last_activity_time > 30000) {
                ...
                if (current_ss_mode == SS_GRNSLD && splash_logo != NULL) {
                    ...
                    lv_coord_t sw = 800; lv_coord_t sh = 480;
    ...
    if (is_idle && current_ss_mode == SS_GRNSLD) {
        ...
        if (current_time - last_ss_frame >= 20) {
            ...
            if (splash_logo != NULL) {
                lv_coord_t sw = 800; lv_coord_t sh = 480;
                lv_coord_t lw = lv_obj_get_width(splash_logo); lv_coord_t lh = lv_obj_get_height(splash_logo);
                if (lw == 0) lw = 150; if (lh == 0) lh = 150;
                ss_logo_x += ss_logo_dx; ss_logo_y += ss_logo_dy;
    ...
    if (active_btn != BTN_NONE) {
        ...
        if (is_booting) {
            is_booting = false;
            lvgl_port_lock(-1); lv_scr_load(ui_RoastMain); lvgl_port_unlock();
        } 
        else if (is_idle) {
            is_idle = false;
            lvgl_port_lock(-1);
...
int main(void) {
    ...
    // Run at a stable frame rate, allowing the browser to breathe
